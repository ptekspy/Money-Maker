...
function compareSchema(
  ...
  const oldEnum = stringArray(oldSchema.enum);
  const newEnum = stringArray(newSchema.enum);
  const removedEnum = oldEnum.filter((item) => !newEnum.includes(item));
...
export function compareContracts(
  ...
  for (const [path, oldPathValue] of Object.entries(oldPaths)) {
    ...
    for (const method of HTTP_METHODS) {
      ...
      const oldParameters = parametersFor(oldDoc, oldPath, oldOperation);
      const newParameters = parametersFor(newDoc, newPath, newOperation);
      const oldParameterKeys = new Set(
        oldParameters.map((parameter) => `${parameter.in}:${parameter.name}`),
      );
